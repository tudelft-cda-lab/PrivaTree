...
def racing_sample(log_terms, random_state):
    ...
    """
    return np.argmin(
        np.log(np.log(1.0 / random_state.uniform(size=log_terms.shape))) - log_terms
    )
...
def opt_comp_calculator(overall_eps, overall_delta, num_comps):
    ...
    for eps_idx in range(num_eps_i):
        ...
        for ell in range(num_comps + 1):
            t_ell_star = np.clip(
                (overall_eps + (ell + 1) * eps) / (num_comps + 1), 0.0, eps
            )
            p_t_ell_star = opt_comp_p(eps, t_ell_star)
            term_sum = 0
            for i in range(num_comps + 1):
                term_sum += (
                    scipy.special.binom(num_comps, i)
                    * np.power(p_t_ell_star, num_comps - i)
                    * np.power(1 - p_t_ell_star, i)
                    * max(
                        np.exp(num_comps * t_ell_star - (i * eps))
                        - np.exp(overall_eps),
                        0,
                    )
                )
...
def ind_exp(sorted_data, data_low, data_high, qs, divided_eps, swap):
    ...
    for q_idx in range(num_quantiles):
        ...
        outputs[q_idx] = np.random.uniform(
            sorted_data[idx_left], sorted_data[idx_left + 1]
        )
